...
- **`gemini.service.ts`'s `MODEL_NAME` is `"gemini-flash-latest"`, not `"gemini-2.0-flash"`.** `gemini-2.0-flash` was deprecated 2026-02-18 (shuts down 2026-06-01) and returns `429 limit: 0` for the free tier; its direct replacement `gemini-2.5-flash` returned `404 ... no longer available to new users` for this API key. `gemini-flash-latest` is the alias that resolves to Google's current recommended Flash model and is confirmed working (verified with a real Gemini call in Task 12). If this breaks again, run `curl "https://generativelanguage.googleapis.com/v1beta/models?key=$GEMINI_API_KEY"` to list currently available models for the key before picking a replacement.
- **Task 11's brief-specified test calls `seedOrderWithDraft()` (from Task 9) twice in one test**, but `Customer.email` and `VerifiedProfessional.email` are `@unique` — the helper's original hardcoded emails collide on the second call. Fixed by hoisting `seedOrderWithDraft()` to module scope in `test/routes/orders.test.ts` and appending a `crypto.randomUUID()` suffix to both emails. Task 9's existing tests are unaffected (only the helper's internals changed, not its `{ order, professional }` return shape). Any later reuse of this helper across multiple calls in one test should rely on the unique emails, not assume fixed values.
